...
fn log_csr_mask_summary(name: &str, row_ptr: &[u32], sequence_length: usize) {
    let nnz = row_ptr[sequence_length] as usize;
    let dense_elements = sequence_length * sequence_length;
    let density = if dense_elements > 0 {
        nnz as f64 / dense_elements as f64
    } else {
        0.0
    };
    info!(
        "  {name}: nnz={}, csr_row_ptr_u32={} bytes, density={:.4}",
        nnz,
...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    info!("\nCSR attention masks for seq[0]:");
    let s = batch.sequence_length;
    log_csr_mask_summary("outbound", &batch.outbound_csr_row_ptr[..s + 1], s);
    log_csr_mask_summary("inbound", &batch.inbound_csr_row_ptr[..s + 1], s);
    log_csr_mask_summary("column", &batch.column_csr_row_ptr[..s + 1], s);
